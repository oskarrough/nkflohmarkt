...
bookingBoxes.forEach(box => {  
		box.addEventListener('click', function (event) {
			// When the customer clicks on the button, redirec
			// them to Checkout.
			...
			stripe.redirectToCheckout({
				items: [{sku: skuCode, quantity: 1}],
			 	...
			 	// Note that it is not guaranteed your customers will be redirected to thi			    // URL *100%* of the time, it's possible that they could e.g. close the
		 	...
		 	.then(function (result) {
				if (result.error) {
					// If `redirectToCheckout` fails due to a browser or networ    k
					// error, display the localized error message to your custo    mer.
